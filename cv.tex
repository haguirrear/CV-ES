...
\section{\raisebox{-.3\baselineskip}{\introductionLogo}Reseña}
  Bachiller en Ingeniería Mecatrónica con interés en temas de Machine Learning, Data Science, Internet de las Cosas y  Desarrollo de software. Dominio de los idiomas inglés y alemán, lenguajes de programación (C, C++, Python, Dart, Javascript) y software de Business Analytics (Power BI), así como de software CAD (Inventor, Fusion 360, AutoCAD). En búsqueda de aportar con la sociedad generando un impacto positivo con los proyectos que realice. Aficionado a la fotografía, lectura y deporte.
  

%Section: Work Experience at the top
\section{\raisebox{-.3\baselineskip}{\workLogo}Experiencia de Trabajo}
\begin{longtable}{r|p{11cm}}
  \emph{Ago 2021 - A la fecha} & Backend Tech Lead \\ & \emph{Crehana}\\&\footnotesize{Lidero los equipos de Workplace Apps y Partnerships en los que trabajamos para integrar tanto el contenido de otras plataformas en Crehana, como para exportar el contenido de Crehana a otros plataformas LMS. Para lograr eso usamos una arquitectura de microservicios programados en Python y GraphQL para comunicarse con los clientes. Además de distintas herramientas de AWS como Lambdas, Step Functions, SNS y SQS para tareas asíncronas }\\&\\
  \emph{Mar 2021 - Jul 2021} & Backend Engineer \\ & \emph{La Positiva - Laboratorio de Transformación Digital}\\&\footnotesize{Participo en varios proyectos que tienen como objetivo facilitar la venta y el acceso a los productos de la positiva tanto para usuarios finales como para empleados de La Positiva. Entre ellos se encuentra Mobility, una app móvil, en donde el backend usa \emph{Django} (\emph{Python}) y los servicios de AWS.}\\&\\

  \emph{Dic 2020 - Mar 2021} & Desarrollo de una aplicación móvil de capacitación sobre el COVID para la primera linea en las regiones de la selva del Perú. \\& \emph{GIZ (Sociedad Alemana de Cooperación Internacional)}\\ & \footnotesize{Como parte de uno de los proyectos de GIZ se tenía el proveer al \emph{MINSA} (Ministerio de Salud del Perú) de una aplicación móvil que permitiera la capacitación del personal de salud de primera linea en las comunidades nativas de la Selva del Perú. Fui contratado como consultor junto a Karlo Verde (Compañero desarrollador) para realizar la aplicación móvil. Yo me encargué de desarrollar el backend (Se usó \emph{Python} y \emph{Fastapi} como framework) que contiene toda la información que necesita la aplicación móvil y su despliegue y mantenimiento en un servidor (Usando \emph{Docker} y \emph{AWS}). Además también desarrollé la aplicación móvil para android usando \emph{Flutter}. 
  }\\&\\
  
  \emph{Dic 2019 - Feb 2021} & Backend Developer \\&\emph{Simple Perú}\\&\footnotesize{Simple Perú es una empresa que se brinda soluciones tecnológicas a medida (Aplicaciones web, APIs REST, aplicaciones móviles y proyectos de IoT). Al trabajar aquí he tenido la oportunidad de desarrollar REST APIs para diferentes proyectos usando \emph{Python}, \emph{Flask} y \emph{PostgreSQL}. Además desarrollé también aplicaciones móviles tanto para Android como para iOS usando \emph{Flutter}. Participé también en el despliegue y mantenimiento de las aplicaciones en los servidores, tanto en ambientes de desarrollo como de producción, usando \emph{Docker} y los servicios de \emph{AWS}.}\\&\\
...
\textsc{Lenguajes de Programación} & \textbullet \hspace{0.5em}\textit{Python:} Implementación de algoritmos de machine learning e \hspace*{0.95em}Inteligencia Artificial y desarrollo de APIs REst usando \emph{Flask} y \emph{FastAPI}.\\
...
& \textbullet \hspace{0.5em}\textit{Javascript:} Desarrollo de APIs REST con NodeJs (Básico). \\
\end{tabular}
\vspace{1em}
...
\multirow{2}{3cm}{\raggedleft\textsc{Conocimiento en Software}} 
...
& \textbullet \hspace{0.5em}\textit{AWS:} Nivel Intermedio. Despliegue de servicios Backend y uso de sus principales productos (EC2, S3, ECR, etc.).\\
...
&  \textbullet \hspace{0.5em}\textit{Word, Powerpoint y Excel:} Nivel Avanzado. \\
